		...
		$('#delete-confirm').dialog({
			...
			buttons: {
				Delete: function () {
					$(this).dialog('close');
					$.ajax({
						url: '../deletePayment/' + $this.data('payment-id'),
						type: 'POST',
...
<div class="invoice-options">
	<a id="print-invoice" href="#print">Print Invoice</a> | <?= anchor("/invoice/index/'.$id.'?format=csv", 'Download as csv file') ?></a>
</div>
<hr />
<br />
<div>
	<fieldset>
		...
		<br />
		<a id="add-payment" class="print-hide" data-invoice-id="<?=$id?>" href="#">Add a Payment</a>
		<br />
		<br />
...
<div class="formatted-content invoice-header">
	<div>
		<label>Invoice ID: </label><span><?=$id?></span>
	</div>
	<div>
		<label>Invoice Date: </label><span><?=$invoicedate?></span>
	</div>
	<div>
	...
	<div>
		...
		<span data-amount="<?=$amountpaid?>" class="amount-paid">$<?=$amountpaid?></span>
	</div>
	<div>
		<label>Outstanding Balance: </label>
		<span data-balance="<?=$outstandingbalance?>" 
			  class="outstanding-balance <? if ($outstandingbalance > 0) echo 'not-paid-in-full'; ?>">$<?=$outstandingbalance?></span>
	</div>
</div>
<div>
	<fieldset>
